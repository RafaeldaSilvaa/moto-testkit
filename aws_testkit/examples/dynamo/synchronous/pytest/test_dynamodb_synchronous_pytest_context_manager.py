...
def test_put_and_get_item_with_context_manager() -> None:
    """Insert and get an item using MotoTestKit as context manager."""
...
def test_query_items_with_context_manager() -> None:
    """Insert and query items using context manager style."""
    with AutoMotoTestKit(auto_start=True, patch_aiobotocore=False) as kit:
        repo = DynamoDBRepository(endpoint_url=kit.get_client("dynamodb").meta.endpoint_url)
        repo.create_table(
            table_name="Users",
