...
class DynamoDBAsyncRepository:
    ...
    async def create_table(self, table_name: str, key_schema: List[Dict], attribute_definitions: List[Dict], provisioned_throughput: Dict):
        async with aioboto3.Session().client("dynamodb", endpoint_url=self.endpoint_url, region_name=self.region_name) as client:
