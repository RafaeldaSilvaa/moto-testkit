...
from src import DynamoItemModel, S3ObjectModel, SQSMessageModel
from src.decorator import use_moto_testkit
...
class MyFullAsyncTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_async(self, moto_testkit):
        ...
        tables = (
            await (await moto_testkit.get_async_client("dynamodb")).list_tables()
        )["TableNames"]
        ...
        queues = (await (await moto_testkit.get_async_client("sqs")).list_queues())[
            "QueueUrls"
        ]
...
class TestFullIntegration(unittest.TestCase):
    ...
    def test_sync_example(self, moto_testkit):
        ...
        self.assertIn(
            "bucket-class-sync", [b["Name"] for b in s3.list_buckets()["Buckets"]]
        )
